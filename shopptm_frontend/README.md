...
### 저장소에 남아있던 낡은 백엔드 빌드 산출물
- **문제**: 프론트엔드 저장소 루트에 이전에 실수로 복사된 백엔드(Spring Boot) 정적 빌드 산출물 폴더가 그대로 남아있었음.
- **원인**: 빌드 산출물을 백엔드 저장소로 복사하는 과정에서 잘못된 경로에 압축 해제된 것으로 추정.
- **해결**: 불필요한 산출물 폴더를 삭제해 저장소 디스크 용량과 혼동 요소를 정리.
