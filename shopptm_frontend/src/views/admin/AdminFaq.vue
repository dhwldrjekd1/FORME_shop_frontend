  ...
  <AdminLayout>
    <div class="af">
      <div class="af-head">
        ...
        <button class="af-add" @click="openCreate">+ FAQ 추가</button>
      </div>
        ...
        <table class="af-table">
          ...
          <tbody>
            <tr v-for="f in items" :key="f.id">
              ...
              <td class="t-actions">
                <button class="t-btn" @click="openEdit(f)">수정</button>
                <button class="t-del" @click="del(f.id)">삭제</button>
              </td>
            </tr>
      ...
      <Teleport to="body">
        <div v-if="showModal" class="modal" @click.self="showModal = false">
          <div class="modal__box">
            ...
            <input v-model="form.question" type="text" placeholder="질문" class="modal__input" />
            <textarea v-model="form.answer" rows="4" placeholder="답변을 입력하세요"></textarea>

            <label class="modal__label">정렬 순서 (작을수록 먼저 표시)</label>
            <input v-model.number="form.sortOrder" type="number" class="modal__input" />

            <div class="modal__actions">
              <button class="modal__btn modal__btn--ghost" @click="showModal = false">취소</button>
              <button class="modal__btn modal__btn--fill" @click="submit">저장</button>
            </div>
          </div>
        </div>
      </Teleport>
